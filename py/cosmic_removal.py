...
for n in files:
    try:
        ...
        continue
    try:
        fitsfile[0].header["HIERARCH ESO DPR CATG"] = fitsfile[0].header[
            "HIERARCH ESO DPR CATG"
        ]
    except:
        continue

    if fitsfile[0].header["HIERARCH ESO DPR CATG"] == "SCIENCE" and (
        fitsfile[0].header["HIERARCH ESO SEQ ARM"] == "UVB"
        or fitsfile[0].header["HIERARCH ESO SEQ ARM"] == "VIS"
        or fitsfile[0].header["HIERARCH ESO SEQ ARM"] == "NIR"
    ):
