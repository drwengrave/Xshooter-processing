...
Originally written by Thomas Krühler
20.08.2017 - Adapted to Python 3 and adjusted by Jonatan Selsing
...
class molecFit:
    ...
    def __init__(self, fitsfile_path, object_name, output_path, vac_air):
    ...
    def setParams(self):
        ...
        prange = os.path.join(
            molecBase, "examples/XSHOOTER/exclude_xshoo_%s.dat" % self.arm
        )
        ...
        self.molecparfile = os.path.abspath(
            self.output_path + "/%s_molecfit_%s.par" % (self.name, self.arm)
        )
    ...
    def updateSpec(self, tacfile=""):
        ...
        if tacfile == "":
            os.rename(
                self.output_path
                + self.input_file_path.split("/")[-1].split(".")[0]
                + "_TAC.fits",
                self.output_path + self.name + "_TAC.fits",
            )
        ...
        # wrange = os.path.join(molecBase, 'examples/config/include_xshoo_%s.dat' % self.arm)
        wrange = os.path.abspath(
            self.output_path + "/%s_molecfit_%s_inc.dat" % (self.name, self.arm)
        )
        g = open(wrange, "r")
        ...
        for fitreg in fitregs:
            mictowl = 1.0 / self.params["wlgtomicron"]
            if float(fitreg.split()[1]) * mictowl < self.wave[-1]:
                ...
                cont = np.median(tcspecp[transmp > 0.90]) / np.median(
                    transmp[transmp > 0.90]
                )
                ...
                for ax in [ax1, ax2]:
                    ...
                    ax.set_xlim(
                        float(fitreg.split()[0]) * mictowl,
                        float(fitreg.split()[1]) * mictowl,
                    )
                ...
                ax1.set_ylabel(
                    r"$F_{\lambda}\,\rm{(10^{-%s}\,erg\,s^{-1}\,cm^{-2}\, \AA^{-1})}$"
                    % (str(1e17))[-2:]
                )
                if len(tcspecp[transmp > 0.90]) > 3:
