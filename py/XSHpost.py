...
class XSHorder_object(object):
    ...
    def __init__(self, order_flist, merge_flist, sky_flist, sdp_flist):
        ...
        fwhm = np.nanmean([np.nanmean([ii[0].header["HIERARCH ESO TEL AMBI FWHM START"], ii[0].header["HIERARCH ESO TEL AMBI FWHM END"]]) for ii in self.hdul])

        # Correct seeing for airmass
        airm = np.nanmean([np.nanmean([ii[0].header["HIERARCH ESO TEL AIRM START"], ii[0].header["HIERARCH ESO TEL AIRM END"]]) for ii in self.hdul])
    ...
    def make_empty_order_out_frame(self):
        ...
        # Get physical axes
        self.waveaxis_list = [np.arange(self.naxis1_list[nn])*self.cdelt1 + self.crval1_list[nn] for nn in range(self.norders)]
        self.spataxis_list = [np.arange(self.nypixel)*self.cdelt2 + self.header['CRVAL2'] - 2 * self.cumoff[0] for nn in range(self.norders)]
    ...
    def merge_images(self):
            ...
            for ii in range(self.nfiles):
                ...
                if np.isnan(copy.deepcopy(err_temp)).all():
                    ...
                    for kk in numba.prange(self.naxis1):
                        err_bin = 1/(err_temp[:, kk-4:kk+4][~(mask[:, kk-4:kk+4])])**2
                        if len(err_bin) != 0:
                            run_var[kk] = np.median(err_bin.flatten())
                    err_smooth_tiled = np.tile(np.nanmedian(run_var, axis=0), (self.naxis2, 1))
        ...
        # weight_cube = weight_cube/np.tile(np.sum(weight_cube, axis=2).T, (shp[2], 1, 1)).T
                ...
                self.err_order_new[nn][cut_slice, :] += weight_temp**2. * err_temp**2.
            ...
            err_scaled = np.sqrt(self.err_order_new[nn] / weight_map[nn]**2.)
    ...
    def make_empty_binned_order_frame(self):
        ...
        self.bin_nxpixel = (np.floor((np.array(self.end_wave_list) - np.array(self.start_wave_list))/ (self.cdelt1*self.bin_factor))).astype("int") + 1
        # Pick refence wcvs
        self.ref_wcs = self.wcs_outer[0][-1]
    ...
    def bin_orders(self):
        ...
        for nn in numba.prange(self.norders):
            ...
            bin_flux, bin_error, bin_bpmap = bin_image(self.hdu[nn*3].data, self.hdu[nn*3 + 1].data, self.hdu[nn*3 + 2].data, self.bin_factor, weight = False)
    ...
    def write_binned_fits(self):
        ...
        for ii, kk in enumerate(self.hdu):
            ...
            self.hdu[ii].header["CD1_1"] = self.hdu[ii].header["CD1_1"]*self.bin_factor
    ...
    def subtract_residual_sky(self):
        ...
        for nn in range(self.norders):
            ...
            for ii in numba.prange(len(self.waveaxis_list[nn])):
                ...
                try:
                    chebfit = chebyshev.chebfit(self.spataxis_list[nn][~mask], vals, deg = 2, w=1/errs)
                    ...
                    chebfitval = np.ones_like(self.spataxis_list[nn])*np.ma.median(self.spataxis_list[nn][~mask])

                if ii % 100 == 0 and ii != 0:
                    # Plotting for quality control
                    pl.errorbar(self.spataxis_list[nn][~mask], vals, yerr=errs, fmt=".k", capsize=0, elinewidth=0.5, ms=3)
                    pl.plot(self.spataxis_list[nn], chebfitval)
                    pl.xlabel("Spatial index")
                    pl.ylabel("Flux density")
                    pl.title("Quality test: Sky estimate at index: "+str(ii)+" for order: "+str(nn))
                    pp.savefig()
                    pl.clf()
                    # pl.show()
    ...
    def finetune_wavlength_solution(self):
        ...
        for nn in numba.prange(self.nfiles):
            ...
            self.correction_factor[nn] = 1. + offsets[max_idx]
            print("Found preliminary velocity offset: "+str((self.correction_factor[nn] - 1.)*3e5)+" km/s")
            print("")
            ...
            print("Found refined velocity offset: "+str((self.correction_factor[nn] - 1.)*3e5)+" km/s")
            print("")

            # Mask flux with > 3-sigma sky brightness
            self.sky_mask = f(haxis*self.correction_factor[nn]) > np.percentile(f(haxis*self.correction_factor), 99)
            ax2 = ax1.twiny()
    ...
    def fit_order_spsf(self):
        ...
        for nn in range(self.norders):
            ...
            waveaxis = np.arange(xlen)*self.cdelt1*self.bin_factor + self.crval1_list[nn]
            full_waveaxis = np.arange(self.naxis1_list[nn])*self.cdelt1 + self.crval1_list[nn]

            ylen = np.shape(flux)[0]
            spataxis = np.arange(ylen)*self.cdelt2 + self.header['CRVAL2'] - 2 * self.cumoff[0]

            # Inital parameter guess
            fwhm_sigma = 2. * np.sqrt(2.*np.log(2.)) #Conversion between header seeing value and fit seeing value.
            if self.p0 == None:
                self.p0 = [abs(1e1*np.nanmean(flux[flux > 0])), np.median(self.nypixel), abs(self.header['HIERARCH ESO TEL AMBI FWHM START']), 0, 0]
                if two_comp:
                    self.p0 = [1e1*np.nanmean(flux[flux > 0]), np.median(self.nypixel), abs(self.header['HIERARCH ESO TEL AMBI FWHM START']), 0, 0, 5e-1*np.nanmean(flux[flux > 0]), np.median(self.nypixel) + 2, 0.5, 0.1]

            if self.broken_adc:
                dR = ADC_corr_guess()

            # Parameter containers
            amp, cen, fwhm = np.zeros_like(waveaxis), np.zeros_like(waveaxis), np.zeros_like(waveaxis)
            eamp, ecen, efwhm = np.zeros_like(waveaxis), np.zeros_like(waveaxis), np.zeros_like(waveaxis)

            # Loop though along dispersion axis in the binned image and fit a Voigt
            x = np.arange(min(spataxis[width:-width]), max(spataxis[width:-width]), 0.01)
            inp_cent = self.p0[1]
            ...
            for ii in range(len(waveaxis)):
                try:
                    ...
                    # Fit SPSF
                    if self.two_comp:
                        popt, pcov = optimize.curve_fit(Two_Moffat1D, spataxis[width:-width], flux[:, ii][width:-width], p0 = self.p0, maxfev = 5000)
                    elif not self.two_comp:
                        popt, pcov = optimize.curve_fit(Moffat1D, spataxis[width:-width], flux[:, ii][width:-width], p0 = self.p0, maxfev = 5000)

                    pl.errorbar(spataxis[width:-width], flux[:, ii][width:-width], yerr=error[:, ii][width:-width], fmt=".k", capsize=0, elinewidth=0.5, ms=3)
                    if self.two_comp:
                    ...
                    pl.title("Profile fit in order: "+str(nn)+" at index: "+str(ii))
            ...
            while std_resid > 0.5:
                ...
                fitcen = chebyshev.chebfit(waveaxis[idx], cen[idx], deg=self.pol_degree[0], w=1/ecen[idx])
            ...
            ax1.errorbar(waveaxis, cen, yerr=ecen, fmt=".k", capsize=0, elinewidth=0.5, ms=7)
            ...
            fitfwhm = chebyshev.chebfit(waveaxis, fwhm, deg=self.pol_degree[1], w=1/efwhm)
            fitfwhmval = chebyshev.chebval(full_waveaxis, fitfwhm)
            # Ensure positivity
            fitfwhmval[fitfwhmval < 0.1] = 0.1

            # Plotting for quality control
            ax2.errorbar(waveaxis, fwhm, yerr=efwhm, fmt=".k", capsize=0, elinewidth=0.5, ms=7)
            ...
            mask = ~(eamp == 1e10)
            f = interpolate.interp1d(waveaxis[mask], amp[mask], bounds_error=False, fill_value="extrapolate")
            ...
            self.full_profile[nn], self.trace_model[nn] = np.zeros_like(self.data_order_new[nn]), np.zeros_like(self.data_order_new[nn])
            for ii, kk in enumerate(full_waveaxis):
                self.trace_model[nn][:, ii] = Moffat1D(spataxis, fitampval[ii], fitcenval[ii], fitfwhmval[ii])
                self.full_profile[nn][:, ii] = self.trace_model[nn][:, ii] / abs(np.trapz(self.trace_model[nn][:, ii]))
    ...
    def extract_order_spectra(self):
        ...
        for nn in numba.prange(self.norders):
            # In zero based coordinates
            start_pix = int(np.floor(self.ref_wcs.wcs_world2pix(self.start_wave_list[nn],0 ,0)[0]))
            end_pix = int(np.floor(self.ref_wcs.wcs_world2pix(self.end_wave_list[nn],0 ,0)[0]))
            ...
            bg_variance = signal.medfilt(np.tile(np.nanmedian(err_temp**2., axis=0), (self.nypixel, 1)), [1, 11])

            # Get first extractions
            denom = np.nansum((self.full_profile[nn]**2. / bg_variance), axis=0)
            spectrum = np.nansum(self.full_profile[nn] * data_temp / bg_variance, axis=0) / denom
            errorspectrum_syn = np.sqrt(1 / denom)

            # Create synthetic variance based on error spectrum and profile
            syn_variance = np.tile(errorspectrum_syn**2, (self.nypixel, 1))*self.full_profile[nn] + bg_variance

            # Repeat extraction
            denom = np.nansum((self.full_profile[nn]**2. / syn_variance), axis=0)
            spectrum = np.nansum(self.full_profile[nn] * data_temp / syn_variance, axis=0) / denom
            denom_out = np.nansum((self.full_profile[nn]**2. / err_temp**2.), axis=0)
            ...
            merged_optimal_error_spectrum[cut_slice] += weight_temp**2. * errorspectrum**2.
            ...
            extract_aperture = slice(int(np.floor(cen_idx - self.seeing_pix)), int(np.ceil(cen_idx + self.seeing_pix)))

            # Do normal sum
            s_spectrum = np.nansum(data_temp[extract_aperture, :], axis=0)
            s_errorspectrum = np.sqrt(np.nansum(err_temp[extract_aperture, :]**2, axis=0))
            ...
            merged_standard_error_spectrum[cut_slice] += s_weight_temp**2. * s_errorspectrum**2.
        ...
        self.optimal_error_spec = np.sqrt(merged_optimal_error_spectrum / merged_optimal_weight_map**2.)

        # Scale by the summed weights.
        self.standard_flux_spec = merged_standard_spectrum / merged_standard_weight_map
        self.standard_error_spec = np.sqrt(merged_standard_error_spectrum / merged_standard_weight_map**2.)



        test_file = fits.open(glob.glob("/Users/jonatanselsing/Work/DATA/test_data/reduced_data/OB1_NOD/UVB/*/*SLIT_FLUX_IDP*.fits")[0])
        ...
        pl.plot(wave, self.standard_flux_spec/self.standard_error_spec, label = "aperture")
    ...
    def bin_merged(self):
        ...
        # Make binned spectrum
        bin_flux, bin_error, bin_bpmap = bin_image(self.merge_hdu[0].data, self.merge_hdu[1].data, self.merge_hdu[2].data, self.bin_factor, weight = False)
    ...
    def write_binned_merged_fits(self):
        ...
        for ii in range(len(self.merge_hdu)):
            self.merge_hdu[ii].header["CDELT1"] = self.merge_hdu[ii].header["CDELT1"]*self.bin_factor
            self.merge_hdu[ii].header["CD1_1"] = self.merge_hdu[ii].header["CD1_1"]*self.bin_factor

        # Write fits file
        self.merge_hdu.writeto(self.base_fname_out+"_merged_binned.fits", overwrite=True)


    def run_combine(self, base_fname_out, clean, NOD, bin_factor, p0, broken_adc, two_comp, pol_degree, trace_mask):
...
def main():
    ...
    sky_files = sorted(glob.glob(object_path+"/reduced_data/"+OB+"/"+arm+"/*/*/*SKY_SLIT_MERGE1D_"+arm+".fits"))
    sdp_files = sorted(glob.glob(object_path+"/reduced_data/"+OB+"/"+arm+"/*/*SCI_SLIT_FLUX_IDP_"+arm+".fits"))
    ...
    order_2dfiles.run_combine("%s/%s%s"%(object_path, arm, OB), clean=False, NOD=False, bin_factor=50, p0=[1e-1, -2.5, 1.5, 0, 0], broken_adc=False, two_comp=False, pol_degree=[3, 2], trace_mask = [0])
